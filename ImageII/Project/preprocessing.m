...
for file_no=[12 36 37 38]
    filename='Data/glassmir'+string(file_no)+".hdf5";
    h5disp(filename)
    cells=h5read(filename,'/data');
    counts= size(cells);
    count = counts(4);
    processed_cells= zeros(80,80,1,count*4);
    processed_no=0;
...
%       cell_rotate= imrotate(cell,90);
%       processed_no=processed_no+1;
%       processed_cells(:,:,1,processed_no)=cell_rotate;
      
      mirror_horizontal = flip(cell ,2);
      %figure,imshow(mirror_horizontal);
      processed_no=processed_no+1;
